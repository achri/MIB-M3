...
$(document).ready(function()  {
	$('.editneg').editable('index.php/<?php echo $link_controller;?>/negara_update',{
		indicator : 'Saving...',
	...
	function del_lokasi(id,table) {
		...
		.dialog('option','buttons', {
			...
			"OK" : function() {
				$.post('index.php/<?php echo $link_controller;?>/del_lokasi/'+id+'/'+table,function(data){
					if (data == 'OK') {
	...
	function open_prov(i){
		...
		$.ajax({
			type:'POST',
			url:'index.php/<?php echo $link_controller;?>/daftar_provinsi/'+i,
	...
	function open_kota(i){
		...
		$.ajax({
			type:'POST',
			url:'index.php/<?php echo $link_controller;?>/daftar_kota/'+i,
			success: function(data) {
				$('#r1_'+i).html(data);
				$('#t1_'+i).val('1');
				$('#tr1_'+i).show();
