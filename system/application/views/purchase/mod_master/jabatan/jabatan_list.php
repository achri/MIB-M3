...
$("#dialog_hapus").dialog({
		...
		'<?=$this->lang->line('jquery_button_hapus');?>': function() {			
			...
			$.ajax({
				type: 'POST',
				url: 'index.php/<?php echo $link_controller;?>/jabatan_delete',
				data: "id="+tmp_id,						
	  			dataType:"html"						  					  			
			});								
...
$("#dialog_hapus_berhasil").dialog({
	...
	function deletejab(id){
		$.ajax({
			type: 'POST',
			url: 'index.php/<?php echo $link_controller;?>/jabatan_cek/'+ id,
			success: function(data) {
				if (data == '0'){					
					...
					//alert ("Jabatan tidak dapat dihapus, data telah digunakan !!!");
				}
  			}
		});
	}

	function editjab(row, id){
		$(row).click(function(){
		$('> span',this).editable('index.php/<?php echo $link_controller;?>/jabatan_update',{
			indicator : '<?=$this->lang->line('jquery_indicator_save');?>',
			tooltip   : '<?=$this->lang->line('jquery_tooltip_clik');?>aaa',
			width : '200px'
		});
		});
	}
...
<div id="dialog_hapus" title="<?=($this->lang->line('jquery_dialog_konfirmasi'));?>">
	<p><?=($this->lang->line('ajax_dialog_hapus'));?></p><p id="nama"></p>
</div>
