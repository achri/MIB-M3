...
<div class="noprint" >
	...
	<table align="center" width="1059" cellspacing="5" cellpadding="5">
		<tr>
			<td width="22"><?=$this->lang->line('tahun');?></td>
			<td width="186">: 
			  <select name="cari_tahun" style="width:150px" onchange="setBulan(this)">
              <option value="0" >
		  ...
		  <td width="173">: 
			...
			<td width="7">&nbsp;</td>
		    <td width="99"><?=$this->lang->line('lap_kontra_bon');?></td>
		    <td width="168">: 
		      <select name="status_kb" style="width:150px" >
              <option value="0" >
              <?=$this->lang->line('combo_box_status_kontra_bon');?>
              </option>
		...
		<tr>
		  ...
		  <td>:
            <select name="cari_kategori" style="width:150px"  >
              <option value="0" >
              <?=$this->lang->line('combo_box_kategori');?>
              </option>
              <?php foreach($data_kategori->result() as $rows):?>
              <option value="<?=$rows->cat_code?>" <?=( $cari_kategori == $rows->cat_id )?('SELECTED="selected"'):('')?>>
              <?=$rows->cat_name?>
              <?
		  ...
		  <td>:
            <select name="cari_pemasok" style="width:150px" >
              <option value="0" >
	      ...
	      <td>&nbsp;</td>
	      <td>&nbsp;</td>
		  <td><input name="cari_status" type="submit" value="<?=$this->lang->line('cari');?>" /></td>
		</tr>
	</table>
	<!--  blm jadi
...
<? if ($cari_tahun != '0' || $cari_bulan != 0 || $cari_kategori != 0 || $status_kb != 0 || $cari_pemasok != 0): ?>
  ...
  <? if ($status_kb!= 0):?>
  <tr>
    <td><?=$this->lang->line('lap_kontra_bon');?></td>
    <td>:</td>
	<? if ($status_kb == 1){?>
	    <td><font color="red" ><?=$this->lang->line('combo_box_status_tutup');?>    </font></td>
	<? } else if ($status_kb == 2){ ?>
	    <td><font color="red" ><?=$this->lang->line('combo_box_status_buka');?>    </font></td>	
	<? }?>
  </tr>
  <? endif;?>
...
<table width="83%"  align="left" border="1" cellpadding="0" cellspacing="1" class="ui-widget-content ui-corner-all"
><? if ($cari_status !=''):?>
     ...
     <?=$data_penerimaan[$i]['sup_name']?>    </td>
		  ...
		  <? 
				if ($data_penerimaan[$i]['con_id'] <> 0 ){
				  echo $this->lang->line('combo_box_status_tutup');
				}else if ($data_penerimaan[$i]['con_id'] == 0 ){
				  echo $this->lang->line('combo_box_status_buka');
				 } else {
				 	echo "-";
				 }
			?>
	     ...
	     <?	if ($data_penerimaan[$i]['cur_symbol'] =='Rp') { ?>
		  ...
		  <? } else { ?>
			  ...
			  <td width="3%" align="right" valign="top"><?=number_format($data_penerimaan[$i]['price'],$this->general->digit_dolar())?></td>
			  <td width="1%" align="center" valign="top">-</td>
			  <td width="3%" align="right" valign="top"><?=number_format($data_penerimaan[$i]['gd_totprice'],$this->general->digit_dolar())?></td>
		  <? }  ?>
  </tr>
