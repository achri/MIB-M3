...
class Tbl_po extends Model {
	...
	function po_list($print_status){
		$sql = "SELECT p.po_id, p.po_no, date_format( p.po_date, '%d-%m-%Y' ) AS po_date, s.sup_name, l.legal_name
			FROM prc_po AS p
			INNER JOIN prc_master_supplier AS s ON s.sup_id = p.sup_id
			INNER JOIN prc_master_legality AS l ON l.legal_id = s.legal_id
			WHERE po_printStat = '".$print_status."' ";
	...
	function get_po_content($id){
		...
		$return['detail'] = $this->db->query("SELECT pr.qty, date_format( pr.rfq_delivery_date, '%d-%m-%Y' ) AS rfq_delivery_date, pr.price, pr.discount, suppro.sup_pro_code,
				...
				(pr.price * pr.qty * ( ( 100 - pr.discount ) /100 ) * 10 / 100 ) AS ppn, 
				(pr.price * pr.qty * ( ( 100 - pr.discount ) /100 ) * 110 / 100) AS sum_ppn,
				pro.pro_name, pro.pro_code, m.satuan_name, m.satuan_format, cur.cur_id, cur.cur_symbol, cur.cur_digit
		...
		$return['footer'] = $this->db->query("SELECT pr.cur_id, cur.cur_symbol, cur.cur_digit, 
				sum( pr.price * pr.qty * ( ( 100 - pr.discount ) /100 )) AS tot_price, 
				sum( pr.price * pr.qty * ( ( 100 - pr.discount ) /100 )* 10 / 100) AS hrg_ppn,
				sum( pr.price * pr.qty * ( ( 100 - pr.discount ) /100 )* 110 / 100) AS tot_ppn, 
				sum( pr.price * pr.qty * ( pr.discount /100 ) ) AS tot_discount
				FROM prc_pr_detail as pr
				INNER JOIN prc_master_currency AS cur ON cur.cur_id = pr.cur_id
				WHERE po_id ='$id'
				GROUP BY cur_id");
	...
	function update_po($id, $count, $tgl, $user){
		...
		$this->db->where($dataw);
		return $this->db->update('prc_po',$datau);
	}


	// BY AHRIE	
	function get_bpb_po($sup_id) {
		return $this->db->query('select s.sup_id, s.sup_name,leg.legal_name, p.po_id, p.po_no, (
	...
	function get_bpb_po_grt($po_no,$sup_id) {
		return $this->db->query('select sup.sup_id,sup.sup_name,leg.legal_name, p.po_id, p.po_no, (
			select count(pro_id) from prc_pr_detail as d
			where p.po_id = d.po_id
		 )as jum_item
		 from prc_po as p 
		 ...
		 where p.po_no like "'.$po_no.'%" and p.sup_id = "'.$sup_id.'" and p.po_id in (select po_id from prc_gr) order by p.po_no'); //p.po_status=1 and 
	}
	
	function get_bpb_po_sup($sup_id) {
		return $this->db->query('select sup.sup_id,sup.sup_name,leg.legal_name, p.po_id, p.po_no, (
			select count(pro_id) from prc_pr_detail as d
			where p.po_id = d.po_id
		 )as jum_item
		 ...
		 inner join prc_master_supplier as sup on sup.sup_id = p.sup_id 
		 left join prc_master_legality as leg on leg.legal_id = sup.legal_id 
		 where p.sup_id = "'.$sup_id.'" and p.po_id in (select po_id from prc_gr) order by sup.sup_name'); //p.po_status=1 and 
	}
	
	function get_bpb_sup_ret() {
		return $this->db->query('select distinct leg.legal_name, s.sup_id,s.sup_name from prc_po as p 
	        inner join prc_master_supplier as s on s.sup_id = p.sup_id
			inner join prc_master_legality as leg on leg.legal_id = s.legal_id
			where po_id in (select po_id from prc_gr) order by s.sup_name');
	}
	
	function get_bpb_sup() {
		return $this->db->query('select distinct s.sup_id,s.sup_name,leg.legal_name,s.sup_status from prc_po as p 
	        inner join prc_master_supplier as s on s.sup_id = p.sup_id
			left join prc_master_legality as leg on leg.legal_id = s.legal_id
			where p.po_status="0" and p.po_printStat=1 order by s.sup_name');
	}
	...
	function get_bpb_po_det($po_id,$stats='') {
		...
		//if ($get_po_id->num_rows() > 0):
			...
			$sql_po_det = 'select pr.auth_no,pr.pr_id,pro.pro_id,sup.sup_name,pr.qty, pr.qty_terima, pr.qty_retur, abs((pr.qty_terima - pr.qty_retur - pr.qty)) as qty_remain,
				 ...
				 ELSE "O.K"
				 END as qty_status,
	             pr.price, (pr.price * pr.qty) as amount, pro.pro_code, pro.pro_name,leg.legal_name,
	             m.satuan_name,m.satuan_format 
				 from prc_pr_detail as pr
				 inner join  prc_master_product as pro on pr.pro_id = pro.pro_id 
				 inner join prc_master_satuan as m on pr.um_id = m.satuan_id
				 inner join prc_master_supplier as sup on pr.sup_id = sup.sup_id
				 left join prc_master_legality as leg on leg.legal_id = sup.legal_id
				 where pr.po_id="'.$po_id.'" order by pr.pr_reqTime';
			...
			$sql_po_id = 'select p.po_id, p.po_no, leg.legal_name, date_format(p.po_date,"%d-%m-%Y") as po_date, sup_name from  prc_po as p
	         inner join  prc_master_supplier as s on s.sup_id = p.sup_id
			 left join prc_master_legality as leg on leg.legal_id = s.legal_id
	         where po_id="'.$po_id.'"';
			...
			$sql_gr = 'select distinct d.*, p.pro_code, p.pro_name, g.*, date_format(g.gr_date,"%d-%m-%Y") as gr_date, pr.auth_no, 
			   m.satuan_name, m.satuan_format 
			   ...
			   inner join prc_master_satuan as m on pr.um_id = m.satuan_id
			   where g.po_id = "'.$po_id.'"
			   order by g.gr_date, pr.pr_reqTime';
			
			$return['bpb_gr'] = $this->db->query($sql_gr);
			
			$sql_ret = 'select distinct d.pro_id,d.qty,p.pro_code, p.pro_name, g.*, date_format(g.gr_date,"%d-%m-%Y") as gr_date, pr.auth_no, 
			   pr.price, pr.cur_id, pr.discount, pr.kurs, m.satuan_name, m.satuan_format 
			   from prc_gr_detail as d
			   inner join prc_gr as g on g.gr_id = d.gr_id
			   inner join prc_master_product as p on p.pro_id = d.pro_id
			   inner join prc_pr_detail as pr on pr.po_id = g.po_id and pr.pro_id = d.pro_id
	...
	function get_bpb_gr_form($po_no) {
		...
		if ($get_po_id->num_rows() > 0):
			$po_id=$get_po_id->row()->po_id;
			$return['bpb_po'] = $this->db->query('select leg.legal_name,p.sup_id, p.po_id, p.po_no, po_date, p.cur_id, sup_name from  prc_po as p
	         inner join  prc_master_supplier as s on s.sup_id = p.sup_id
			 inner join prc_master_legality as leg on leg.legal_id = s.legal_id 
	         where po_id="'.$po_id.'"');
			$return['bpb_po_det'] = $this->db->query('select pr.pr_id,pr.qty, (pr.qty - pr.qty_terima + pr.qty_retur) as qty_remain, pr.price, pr.discount, 
             ...
             pro.pro_code, pro.pro_name, pro.pro_id, m.satuan_name, m.satuan_format, pr.um_id as pr_um_id, pro.um_id as pro_um_id, pro.pro_image 
             from  prc_pr_detail as pr
			 inner join  prc_master_product as pro on pr.pro_id = pro.pro_id 
			 inner join prc_master_satuan as m on pr.um_id = m.satuan_id
			 where pr.po_id="'.$po_id.'" order by pr.pr_reqTime');
	...
	function get_po_bon($po_id) {
		return $this->db->query("select pr.qty, pr.qty_terima, pr.qty_retur, abs((pr.qty_terima - pr.qty_retur - pr.qty)) as qty_remain,
             ...
             m.satuan_name,m.satuan_format from prc_pr_detail as pr
			 inner join prc_master_product as pro on pr.pro_id = pro.pro_id 
			 inner join prc_master_satuan as m on pr.um_id = m.satuan_id
