<?php
class List_po_buka extends MY_Controller{
	public static $link_view, $link_controller;
	function List_po_buka(){
		...
		self::$link_controller = 'mod_list/list_po_buka';
		self::$link_view = 'purchase/mod_list/mod_po_buka';
	...
	{		
		$valid_fields = array('po_id','po_no','po_date','sup_name','legal_id','legal_name');
		...
		if ($records['count'] > 0):		
			foreach ($records['result']->result() as $row)
			{
				...
				$row->po_date,
				$row->sup_name,
				'<a href=\'javascript:void(0)\' onclick=\'open_po_buka('.$row->po_id.')\'><img border=\'0\' src=\'./asset/img_source/icon_info.gif\'></a>'
				);
			}
		else: 
	...
	function flexigrid_builder($title,$width,$height,$rp) {
		...
		return build_grid_js('po_buka_list',$ajax_model,$colModel,'po_no','desc',$this->flexi_engine->flexi_params($width,$height,$rp,$title));
	...
	function open_po($po){
		...
		$this->load->view(self::$link_view.'/po_detailview',$data);
	}
