<?php
class service_rfq extends MY_Controller{
	public static $link_view, $link_controller;
	function service_rfq(){
		parent::MY_Controller();
		$this->load->model(array('Tbl_rfq_service','Tbl_purchase_type','Tbl_satuan','Tbl_currency','Tbl_term','Tbl_supplier','Tbl_legal','Tbl_sup_produk','Tbl_category','tbl_satuan_pro'));
		...
		self::$link_controller = 'mod_service/service_rfq';
		self::$link_view = 'purchase/mod_entry/mod_service/mod_rfq_service';
	...
	function index() {
		...
		$grid_js = build_grid_js('flex1',site_url("/mod_service/service_rfqflexigrid"),$colModel,'id','asc',$gridParams);
	...
	function open_srfq($id){
		$data['get_srfq'] =  $this->Tbl_rfq_service->srfq_content($id);
		...
		if ($triger == 0){
			for($i=0;$i<sizeof($pro);$i++) {
				...
				if ($status[$i] == 1){
					...
					//Disetujui
					$result[] = $this->Tbl_rfq_service->srfq_insert_1($rfq, $pr[$i], $pro[$i], $status[$i], $sup[$i], $qty[$i], $sat[$i], $deldate[$i], $harga[$i], $pay[$i], $proname[$i], $kurs[$i], $disc[$i], $cur[$i]);
					}
				} else {
					//Diubah dan disetujui
					$result[] = $this->Tbl_rfq_service->srfq_insert_2_3($rfq, $pr[$i], $pro[$i], $status[$i], $proname[$i]);
				} 
			}
			if ($result){
