<?php
class master_adjustment extends MY_Controller {
	public static $link_view, $link_controller, $link_controller_product, $user_id;
	function master_adjustment() {
		...
		self::$link_controller = 'mod_master/master_adjustment';
		self::$link_view = 'purchase/mod_master/adjustment';
	...
	{		
        ...
        where pro.pro_status = 'active' {SEARCH_STR} group by pro.pro_id";
		if ($cat_code != '')
			$sql .= '';
	...
	function flexigrid_builder($title,$width,$height,$rp,$pro_code='') {
		...
		$colModel['pro_name'] = array($this->lang->line('pro_name'),375,TRUE,'left',2,false,'flexEdit');
	...
	function cek_stokjoin($pro_id) {
		$sql = "select * 
		from prc_master_supplier_product as spro
		inner join prc_master_supplier as sup on sup.sup_id = spro.sup_id
		where spro.pro_id = $pro_id";
