...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		p = $.extend({
			 ...
			 /* oprek ==> */
			 resizableCol: true, //resizble column
			 multisel: true, // multi select row option
			 draggable: true, // draggable column
			 searchitems: true, // search button
			 showButtons: true,
			 showSearch: true,
			 onRowSelect: false, // *** I added  *** 
			 searchText: 'Cari :',
			 clearText: 'Bersihkan',
			 pageText: 'Halaman',
			 ofText: 'dari',
			 ...
			 title: false,
			 pagestat: 'Tampilkan {from} sampai {to} dari {total} items',
			 procmsg: 'Proses sedang dilakukan ...', //'Processing, please wait ...',
			 query: '',
			 qtype: '',
			 nomsg: 'Data tidak ditemukan',
			...
			populate: function () { //get latest data
				...
				if (p.params)
					...
					$.ajax({
					   ...
					   error: function(data) { 
						   try { if (p.onError) p.onError(); } catch (e) {} 
						   /*
						   $('.pPageStat',this.pDiv).html(p.noMsg);
							...
							.click(
								function (e) 
									{ 
										...
										if (p.multisel){ // Multi select row by @HR13 ^^
											$(this).toggleClass('trSelected');
											if (p.singleSelect) $(this).siblings().removeClass('trSelected');
											// OPREK
											if (p.onRowSelect) p.onRowSelect(e,$(this));
										}
									}
							)
							.mousedown(
			 	...
			 	function ()
					{
						...
						.mousedown(function (e) 
							{
								if (p.draggable) { // draggable column ^^
		...
		$(g.bDiv).after(g.pDiv);
		var html = ' <div class="pGroup"> <div class="pFirst pButton"><span></span></div><div class="pPrev pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pcontrol">'+p.pageText+' <input type="text" size="4" value="1" /> '+p.ofText+' <span> 1 </span></span></div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pNext pButton"><span></span></div><div class="pLast pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pReload pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pPageStat"></span></div>';
		...
		if (p.searchitems&&p.showSearch)
			{
				...
				$(g.sDiv).append("<div class='sDiv2'>"+p.searchText+" <input type='text' size='30' name='q' class='qsbox' /> <select name='qtype'>"+sopt+"</select> <input type='button' id='Clear' value='"+p.clearText+"' /></div>");

				$('input[name=q],select[name=qtype]',g.sDiv).keydown(function(e){if(e.keyCode==13) g.doSearch()});
				$('input[id=Clear]',g.sDiv).click(function(){$('input[name=q]',g.sDiv).val(''); p.query = ''; g.doSearch(); });
				$(g.bDiv).after(g.sDiv);				
